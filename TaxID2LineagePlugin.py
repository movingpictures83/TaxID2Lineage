...
class TaxID2LineagePlugin:
  def input(self, inputfile):
    self.parameters = PyIO.readParameters(inputfile)
    self.input = PyPluMA.prefix()+"/"+self.parameters["input"]
    self.input_skiprows = 0
    if ('input_skiprows' in self.parameters):
        self.input_skiprows = int(self.parameters["input_skiprows"])
    self.lineage_file = PyPluMA.prefix()+"/"+self.parameters["lineage_file"]
    self.tax_col = int(self.parameters["tax_col"])
    if ('include_columns' in self.parameters):
        self.include_columns = self.parameters["include_columns"]
    if ('include_columns_names' in self.parameters):
        self.include_columns_names = self.parameters["include_columns_names"]
    self.use_header = "True"
    if ('use_header' in self.parameters):
        self.use_header = self.parameters["use_header"]
    self.ranks = "superkingdom,phylum,class,order,family,species"
    if ('ranks' in self.parameters):
        self.ranks = self.parameters["ranks"]
  ...
  def output(self, outputfile):
    ...
    if self.email:
        ...
        email = None
    if not self.input_skiprows:
        input_skiprows=0
    else:
        input_skiprows=self.input_skiprows
    ...
    if self.lineage_file:
        ...
        #   Read lineage file
        all_lineage_df = pd.read_csv(self.lineage_file, dtype=str)
        #
        ...
        target_df = pd.read_table(self.input, skiprows=input_skiprows, index_col=False, header=useHeader)
        ...
        if self.include_columns:
            target_include_columns_index = self.include_columns.split(",")
        ...
        # Rename columns:
        if self.include_columns_names:
            new_names = self.include_columns_names.split(",")
        ...
        genbank_lineage_df.to_csv(self.outputfile, sep="\t", index=False)
        ...
        if len(to_fetch_df) > 0:
            ...
            if self.include_columns:
                ...
                include_columns_tmp = self.include_columns
                include_cols_names_tmp = self.include_columns_names
    ...
    # If there is no lineage file provided - perform fetching of the whole input through NCBI
        ...
        get_lineage_file(self.input, ranks, self.tax_col, self.include_columns, self.include_columns_names, self.outputfile,
                         self.email)
